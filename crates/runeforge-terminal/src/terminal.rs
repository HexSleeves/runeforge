...
impl Terminal {
    ...
    pub fn put_char(&mut self, pos: IVec2, ch: char, fg: Color, bg: Color) -> bool {
    ...
    pub fn put_string(&mut self, pos: IVec2, text: &str, fg: Color, bg: Color) -> usize {
        let mut count = 0;
        for (i, ch) in text.chars().enumerate() {
            ...
            if self.put_char(IVec2::new(x, pos.y), ch, fg, bg) {
                count += 1;
            }
        }
        count
    ...
    fn get_cell_mut(&mut self, pos: IVec2) -> Option<&mut Cell> {
...
mod tests {
    ...
    fn test_put_char() {
        ...
        assert!(term.put_char(IVec2::new(5, 5), '@', Color::YELLOW, Color::BLACK));
        let cell = term.get_cell(IVec2::new(5, 5)).unwrap();
        assert_eq!(cell.ch, '@');
    ...
    fn test_put_string() {
        ...
        let count = term.put_string(IVec2::new(0, 0), "Hello", Color::GREEN, Color::BLACK);
        assert_eq!(count, 5);

        assert_eq!(term.get_cell(IVec2::new(0, 0)).unwrap().ch, 'H');
        assert_eq!(term.get_cell(IVec2::new(4, 0)).unwrap().ch, 'o');
    }

    #[test]
    fn test_out_of_bounds() {
        let mut term = Terminal::new(10, 10);
        assert!(!term.put_char(IVec2::new(100, 100), '@', Color::WHITE, Color::BLACK));
        assert!(term.get_cell(IVec2::new(100, 100)).is_none());
    }

    #[test]
    fn test_clear() {
        let mut term = Terminal::new(10, 10);
        term.put_char(IVec2::new(5, 5), '@', Color::YELLOW, Color::BLACK);
        term.clear();
        let cell = term.get_cell(IVec2::new(5, 5)).unwrap();
        assert_eq!(cell.ch, ' ');
    }
